...
export function Navlink({ target, text, active }: NavlinkProps) {
    return (
        <Link href={target}>
            <li className={`${active ? "text-main-blue" : "text-white"} text-2xl mx-5`}>
                {text}
            </li>
        </Link>
    );
...
export function HamNavLink({ target, text, active, setIsShown }: HamNavlinkProps) {
    return (
...
export function HamNavLinkLogout({ text, active,setIsShown, handleLogout }: any) {
